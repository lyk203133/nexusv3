<template>
  ...
  <!-- 表格頭 -->
  <div class="grid grid-cols-3 gap-2 p-4 bg-slate-900/50 border-b border-slate-700/50 text-xs font-bold text-slate-400">
  ...
  <div class="divide-y divide-slate-700/50">
    <div 
      ...
      :key="tx.id"
      class="grid grid-cols-12 gap-2 p-4 hover:bg-slate-700/30 transition-colors cursor-pointer items-center"
      @click="viewDetail(tx.id)"
    >
      <!-- 帳號：窄欄，佔 2 欄，並加上截斷 -->
      <div class="col-span-3 text-sm text-white truncate" :title="tx.username || tx.target_id">
        {{ tx.username || tx.target_id || '-' }}
      </div>
      
      <!-- 金額：寬欄，佔 5 欄，數字右對齊 -->
      <div class="col-span-5 text-left font-mono text-emerald-600 font-bold">
        {{ tx.amount_points.toLocaleString() }}
      </div>
      
      <!-- 邀請碼紅利：寬欄，佔 5 欄，數字右對齊 -->
      <div class="col-span-4 text-left font-mono text-emerald-600 font-bold">
...
async function fetchTransactions(reset = true) {
  ...
  try {
    ...
    const response = await api.get('/user/invite-code-bonus-group', { params })
...
function viewDetail(userId){
  router.push('/invite-code-bonus-detail?userId='+userId)
}
</script>
