      ...
      <div v-if="showFilter" class="bg-slate-800/80 p-4 border-b border-slate-700 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select 
              v-model="filters.status" @change="changeStatus"
              class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-sm text-white"
            >
              <option value="all">{{ t.common.all }}</option>
        ...
        <div v-if="activeDatePreset === 'custom'" class="grid grid-cols-2 gap-3">
          <div>
            ...
            <button
              class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-sm text-left text-white flex items-center justify-between"
...
// Applied date range (shown after submit)
...
// dateRange mirrors filters.startDate / filters.endDate for VueDatePicker
...
function applyDatePreset(key) {
  ...
  if (key === 'today') {
    ...
    // keep existing dates, just show the inputs
  }
}
