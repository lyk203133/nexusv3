    ...
    <div class="p-4 flex items-center bg-slate-800 shadow-lg z-10">
      ...
      <div class="flex-1">
        <h2 class="text-white font-bold text-lg">
          {{ isEditing ? (editingId ? '編輯' : t.account.add) : t.profile.receiveAccount }}
        </h2>
      </div>
      <button 
      ...
      <div v-if="!isEditing" class="space-y-4">
        <div 
          ...
          :class="[
            ...
            item.is_receive == 1 ? 'border-emerald-500 ring-1 ring-emerald-500/50 shadow-lg shadow-emerald-500/10' : 'border-slate-700'
          ]"
        >
          <div v-if="item.is_receive == 1" class="absolute top-0 right-0 bg-emerald-500 p-1 rounded-bl-lg">
          ...
          <div class="flex items-center gap-3 mb-4">
            <div>
              <p v-if="item.is_receive == 1" class="text-[10px] text-emerald-500 font-bold uppercase tracking-wider">使用中</p>
            </div>
          </div>
...
//['PGB', 'ICB', 'MB', 'VCB', 'MSB', 'ACB', 'SEAB', 'EIB', 'OCB']
const bankOptions = [
    {
        "code": "ACB",
        "name": "ACB",
...
async function fetchAccountList() {
  ...
  loading.value = true
  try {
    const response = await api.get('/payment-accounts?type=receive')
...
async function handleDelete(id) {
  ...
  try {
    const res = await api.post('/payment-account-delete', { id })
    if (res.data.success) {
      showToast({ type: 'success', message: res.data.message || '刪除成功' })
      accountList.value = accountList.value.filter(item => item.id !== id)
    } else {
      showToast({ type: 'error', message: res.data.error || '刪除失敗' })
    }
  } catch (err) {
...
async function handleSubmit() {
  ...
  try {
    ...
    if (editingId.value) {
      // 更新現有帳戶
      response = await api.post(`/payment-account-update?id=${editingId.value}&type=receive`, fd, {
        headers: {
      ...
      // 創建新帳戶
      response = await api.post('/payment-account-update?type=receive', fd, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
